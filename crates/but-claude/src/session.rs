...
impl Claudes {
    ...
    async fn spawn_claude_inner(
        ...
        // Persist the Claude session ID to the database so hooks can map claude_session_id
        // back to our stable session_id.
        {
            let mut ctx = sync_ctx.clone().into_thread_local();
            if let Err(e) = db::add_session_id(&mut ctx, session_id, claude_session_id) {
                tracing::error!(
                    ...
                    error = %e,
                    "Failed to persist claude_session_id to database - hooks may fail to find session"
                );
            }
        }
